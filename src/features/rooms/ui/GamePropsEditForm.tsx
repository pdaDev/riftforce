...
import { Switch, FormControlLabel, ToggleButtonGroup, ToggleButton } from "@mui/material"
import { useBlurFocus, DraftStage, AVAILABLE_PLAYERS_COUNT, InfoIcon, RoomType } from "../../../shared"
...
interface IProps extends Pick<ComponentProps<typeof EditForm>, 'loading' | 'submitButtonLabel' | 'loadingStatusLabel' | 'declineButtonLabel' | 'notifyAboutChanges'> {
...
export const GameRoomPropsEditForm: FC<IProps> = observer((
    ...
    const { addStage, playersCount, withBan, draftStages, withExtension,
            initData, setPlayersCount, setWithExtension, deleteStage, password,
            setRoomType, type } = props
    ...
    const hasChanges = notifyAboutChanges ? withExtension !== initData.withExtension 
        || playersCount !== initData.playersCount
        || !draftStages.every((stage, i) => stage === initData.draftStages[i])
    : undefined

    const handleSetPlayersCount = (count: any) => () => setPlayersCount(count)

    const handleSetWithExtension: ChangeEventHandler<HTMLInputElement> = e => {
    ...
    const canNotSubmit = !canSubmit || (props.type === RoomType.private && (!props.password.data || !props.password.isValid))
    ...
    return <EditForm onClose={onClose}
        ...
        { AVAILABLE_PLAYERS_COUNT.map(count => (
            <button type="button"
                    key={count}
                    className={`rounded-full w-8 h-8 text-white ${count === playersCount ? 'bg-fuchsia-100' : 'bg-fuchsia-400'}`} 
        ...
        <FormControlLabel 
            sx={{ width: 'auto' }}
            control={
                <Switch checked={withExtension}
                        disabled={withExtensionSwitcherDisabled}
                        onChange={handleSetWithExtension}/>} 
            label="Использовать дополнение" 
        />
        { withExtensionSwitcherDisabled && <InfoIcon message="Не хватает карт" /> }
       
                <h2>
                    Настройка стадий драфта
                </h2>
                    ...
                    </button>
                {
                    isSelectingDraftStage
                     && <div className="flex gap-1 absolute -bottom-20 -left-1 flex-col">
                        <DraftStageAddButton addStage={addDraftStage}
    ...
    </div>
    {
        !hide?.type && (
            <>
                ...
                >
                    <ToggleButton type="button" value={RoomType.public} aria-label="left aligned">
                        Публичная
                    </ToggleButton>
                    <ToggleButton type="button" value={RoomType.private} aria-label="left aligned">
                        Приватная
                    </ToggleButton>
                </ToggleButtonGroup>
      {
        isPrivateRoom && (
            <PasswordInput value={password.data?.toString() || ''}
