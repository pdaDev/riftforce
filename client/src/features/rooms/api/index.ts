...
import {
    CommonResultServerResponse,
    GetEntitiesListPayload,
    HTTPAxiosAPI,
    ResponseResult,
    ServerListResponse,
    StatusAction,
...
export class RoomsAPI extends HTTPAxiosAPI implements RoomsApi {
    ...
    connect = async ({ id, ...payload }: ConnectToRoomPayload) => {
        const res = await this.post<CommonResultServerResponse>(
            `/${id}`,
            payload,
            {
    ...
    getList = async ({ key, ...payload }: GetEntitiesListPayload) => {
        return await this.get<ServerListResponse<ServerRoomCard>>(
            this.transformListRequestPayloadToQuery("", payload),
            {
