...
export const StartLocalGamePopup: FC<IBasePopupProps> = ({ onClose }) => {
    ...
    const startGame = ({
        ...
        if (user) {
            ...
            setService(
                () => new GameState(sessionId, GameType.local, null)
            ).then((service) => {
                onClose()
                service.start({
                    users,
    ...
    return (
        <div className="w-[600px] w-full">
            <h2 className="text-xl font-semibold mb-2">Играть локально</h2>
            <GameRoomPropsEditForm
                onClose={onClose}
                props={props}
                loading={false}
                hide={{
