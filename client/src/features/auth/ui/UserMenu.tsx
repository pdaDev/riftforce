...
export const UserMenu: FC<IProps> = observer(({ goToLogin }) => {
    ...
    const goToMyRooms = () => n((p) => p.rooms.my)

    if (!isAuthorized) {
        return (
            <Button title="Войти" onClick={goToLogin} variant="outlined">
                Войти
            </Button>
        )
    }
    ...
    return (
        <>
            ...
            <Menu
                ...
                <MenuItem onClick={goToMyRooms}>
                    <ListItemIcon>
                        <FaceIcon />
                    </ListItemIcon>
                    <ListItemText>Мои комнаты</ListItemText>
                </MenuItem>
                <Divider />
