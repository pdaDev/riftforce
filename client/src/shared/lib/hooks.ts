...
): [state: boolean, toggle: () => void, setState: (value: boolean) => void] => {
...
export const useNavigationPermission = (
    ...
    const defaultPermission = defaultPermissions
        ? useHasAccess(defaultPermissions)
        : true
...
export const useBlurFocus = (
    hide: Function
): { onFocus: FocusEventHandler; onBlur: FocusEventHandler } => {
...
export function useClassState<T extends object>(
    defValue: T
): [state: T, setState: (value: Partial<T>) => void] {
    const [state, setState] = useState(defValue)
    const setClassState = (value: Partial<T>) =>
...
export const useFileDrop = () => {
    ...
    const onDrop = (onImageChanges: (files: FileList) => void) =>
        ((e) => {
            e.preventDefault()
...
export const useFormMode = (defaultMode?: FormMode, query: boolean = false) => {
    ...
    const [stateFormMode, setStateFormMode] = useState(
        defaultMode || FormMode.view
    )

    const formMode: FormMode = query
        ? (searchParams.get("mode") as FormMode) || FormMode.view
        : stateFormMode
...
export const useQueryRemoteParam = () => {
    ...
    const remoteQueryParam = query.get(REMOTE_KEY)
    const remote = (remoteQueryParam ? Number(remoteQueryParam) : 1) > 0
...
export function useFormController<T extends object>({
    ...
    const checkChanges = () => {
        return (
            !defaultValues ||
            (defaultValues &&
                getObjectKeys(defaultValues).every((key) => {
