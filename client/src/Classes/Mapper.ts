...
export class RoomTeamMapper {
    static toDomain({
        ...
        name,
        users,
    }: ServerTeam<ServerRoomUserState>): Team<UserWithReadyStatus> {
        return new Team(
            id,
            users.map(
                ({ ready, ...user }) =>
                    new UserWithReadyStatus(UserMapper.toDomain(user), ready)
            ),
            name
        )
    }
    ...
    static toEntity({
        ...
        name,
        users,
    }: Team<UserWithReadyStatus>): ServerTeam<ServerRoomUserState> {
        return {
            id,
            name: name || "",
...
export class GameRoomPropsMapper {
    static toDomain({
        ...
        password,
        name,
        props: { playersCount, withExtension, draftStages },
    }: Omit<ServerRoomState, "teams">): GameRoomPropsController {
...
export class MobDataMapper {
    static toDomain(code: MobDataCode): MobData {
        ...
        return new MobData(
            GuildsMapper.toDomain(guild as GuildCode),
            +value,
            +index
        )
    }
...
export class ElementalMapper {
    static toDomain(code: ElementalCode): Elemental {
        return new Elemental(
            MobDataMapper.toDomain(code as MobDataCode),
            +code.split("_")[3]
        )
    }
    ...
    static toEntity({ total, hand, left }: CardsController): ServerUserDecks {
        return {
            ...
            hand: hand.cards.map((card) =>
                MobDataMapper.toEntity(card.mobData)
            ),
...
export class FieldsControllerMapper {
    static toEntity(
        constroller: FieldsController
    ): Map<User, ElementalCode[][]> {
        ...
        constroller.iterateThroughAllNodes((node) => {
            if (node.user) {
                ...
                userFields.push(
                    node.fields
                        .filter((field) => field.elemental)
                        .map((field) => field.elemental!.code)
                )
            }
        })
