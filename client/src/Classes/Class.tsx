...
import {
    ...
    DraftConfig,
    DefaultPicks,
    DefaultProcessGameState,
    PrevState,
    Log,
    LogSideEffect,
    CommandFormatter,
    HighlightElementConfig,
    HighlightElementConfigForGetActions,
    CardWithoutResultHandler,
    EqualFunction,
    FieldHandler,
    FieldHandlerWithNoResult,
    GameProcessUpdateAllStatePayload,
    DraftUpdateAllStatePayload,
    NextTurnSideEffect,
    GameStrategyCodeType,
    StrategyController,
    TimerChangeTimeSideEffect,
    IGameAPI,
    IInGameStageController,
    IStageController,
    InGameCode,
    InGameStageControllerDefaultData,
    InteractionType,
    LogReader,
    Mediator,
    StageCode,
...
export abstract class Guild {
    ...
    protected transferToAny = (
        field: Field,
        fields: FieldsController,
        cb: FieldHandler
    ) => {
        fields.getTeamAnyLastEmptyFieldAsync(
            field.parent.user![1],
            [field.parent],
            (newPlace) => {
...
export class IceGuild extends Guild {
    readonly action = [
        "Если последнему врагу в этой лоакции уже нанесен хотя бы 1 урон, нанесите ему 4 урон",
        "В ином случае нанесите ему 1 урон",
    ]
    ...
    activate = (
        ...
        if (elemental) {
            const damage =
                elemental.elemental!.health < elemental.elemental!.maxHealth
                    ? 4
                    : 1
            killer.hit(field, elemental, damage)
        }
...
export class LightGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона первому врагу в этой локации.",
        "Снимите 1 урон с активированного элементаля Света или с любого другого союзника",
    ]
...
export class FireGuild extends Guild {
    readonly action = [
        "Нанесите 3 урона первому врагу в этой локации.",
        "Нанесите 1 урон союзнику, который выложен сразу за этим элменталем огня",
    ]
    readonly name: string = "Огонь"
    readonly icon: string = ""
    readonly color: string = "red-400"
...
export class CrystalGuild extends Guild {
    readonly action = [
        "Нанесите 4 урона первому врагу в этой локации. Когда элементаль кристалл уничтожается, соперник получает на 1 очко энергии больше",
    ]
    readonly name: string = "Кристал"
    readonly icon: string = ""
    readonly color: string = "purple-800"
...
export class WaterGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона первому врагу в этой локации",
        "Переместите этого элменталя Воды в соседнюю локацию.",
...
export class AirGuild extends Guild {
    readonly action = [
        "Переместите этого элменталя Воздуха в любую другую локацию.",
        "Нанесите 1 урон первому врагу в новой локации, а также обеих соседних",
    ]
    readonly name: string = "Воздух"
    readonly icon: string = ""
    readonly color: string = "slate-200"
    ...
    activate = (
        ...
        this.transferToAny(field, fields, (newPlace) => {
            fields
                .getEnemyFirstElementalsInThreeColsSync(newPlace!)
                .forEach((f) => {
...
export class DarkGuild extends Guild {
    readonly action = [
        "Переместите этого элементаля Тьмы в другую локацию.",
        "Нанесите 1 урон первому врагу в этой локации. Если после этого враг уничтожен, получите на 1 очко энергии больше",
    ]
    readonly name: string = "Тьма"
    readonly icon: string = ""
    readonly color: string = "gray-400"
...
export class FloraGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона первому врагу в соседней локации",
        "Переместите данного врага в локацию, где находится этот элементаль Флоры",
    ]
    ...
    activate = (
        ...
        fields.getNeighBoorEnemyFirstElementalAsync(field, (neighboor) => {
            if (neighboor) {
                const newPlace = fields.getLastFieldInNode(field.parent.up!)
                fields.transferElementalToNewField(neighboor, newPlace)

                killer.hit(field, newPlace, 2)
            }
...
export class LightingGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона любому врагу в этой локации",
        "Если при этом элменталь Молнии уничтожает врага, тут же воспользуйтесь этой способностью еще раз",
    ]
...
export class GroundGuild extends Guild {
    readonly action = [
        "Когда вы призываете этого элменталя Земли, нанесите по 1 урону каждому врагу в этой локации.",
        "При активации нанесите 2 урона первому врагу в этой локации",
    ]
...
export class AcidGuild extends Guild {
    readonly action = [
        "Нанесите 3 урона первому врагу в этой локации.",
        "Нанесите 1 урон второму врагу в этой локации. Если при этом элементаль Кислоты уничтожает врага вы не получаете очков энергии",
    ]
...
export class CometGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона первому врагу в этой локации",
        "Если у вас на руке меньше 7 карт, возьмите 1 карту из своей колоды",
    ]
...
export class LoveGuild extends Guild {
    readonly action = [
        "Когда вы призываете этого элменталя любви, снимите все жетоны урона с одного союзника в этой локации",
        "Нанесите 2 урона первому врагу в этой локации",
    ]
...
export class MusicGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона первому врагу в этой локации",
        "Если при этом элементаль Музыки уничтожает врага, переместите его на вашу сторону разлома в одну из соседних локаций",
    ]
    readonly name: string = "Лед"
    readonly icon: string = ""
    ...
    activate = (
        ...
        if (enemy) {
            ...
            if (enemyElemental.isDead) {
                ...
                fields.getNeighBoorLastEmptyFieldAsync(
                    field,
                    (neighboorPlace) => {
                        fields.transferElementalToNewField(
                            enemy,
                            neighboorPlace
                        )
                        neighboorPlace!.elemental!.guild.spawn(
                            neighboorPlace,
                            fields,
                            killer,
                            cards,
...
export class BeastGuild extends Guild {
    readonly action = [
        "Переместите этого элменталя Оборотня в соседнюю локацию",
        "Если этому элменталю Оборотня уже нанесен хотя бы 1 урон, нанесите 3 урона первому врагу в новой локации. В ином случае нанесите врагу 2 урона",
    ]
    readonly name: string = "Оборотень"
    readonly icon: string = ""
    readonly color: string = "red-900"
    ...
    activate = (
        ...
        this.transferToNeighbors(field, fields, (newPlace) => {
            const damage =
                newPlace.elemental!.health < newPlace.elemental!.maxHealth
                    ? 3
                    : 2
...
export class MagneticGuild extends Guild {
    readonly action = [
        "Нанесите 2 урона последнему врагу в этой локации",
        "Переместите данного врага и этого элементаля Магнетизма в соседнюю локаци",
    ]
    readonly name: string = "Магнетизм"
    readonly icon: string = ""
    readonly color: string = "slate-600"
    ...
    activate = (
        ...
        fields.getNeighBoorLastEmptyFieldAsync(field, (neighbour) => {
            ...
            if (enemy) {
                const newEnemyPlace = fields.getLastFieldInNode(
                    neighbour!.parent.up!
                )
...
export class SandGuild extends Guild {
    readonly action = [
        "Переместите этого элементаля Песка в любую дургую локацию.",
        "Нанесите по 1 урону каждому врагу в новой локации.",
        "Снимите 1 урон с активированного элементаля Песка",
    ]
...
export class LavaGuild extends Guild {
    readonly action = [
        "Нанесите по 2 урона каждому из первых врагов в соседних локациях",
        "Нанесите 1 урон этому элменталю Лвы и всем союзникам, которые выложены перед ним",
    ]
...
export class Field extends SelectiveElement {
    ...
    static parseCode = (
        nodeCode: MapNodeWithUserCode,
        elementalCode: ElementalCode
    ): FullFieldCode => {
    ...
    get code(): FullFieldCode | MapNodeWithUserCode {
        if (this.elemental && this.parent.user) {
            return Field.parseCode(
                this.parent.code as MapNodeWithUserCode,
                this.elemental.code
            )
        }
...
export const LOG_ACTIONS = {
    ...
    spawnBy: (type: InteractionType) => `SPAWNED_BY_${type}`,
    activateBy: (type: InteractionType) => `ACTIVATE_BY_${type}`,
} as const
...
export class Card extends SelectiveElement {
    ...
    get code() {
        return this.mobData.code
    }
}

export class Deck<T extends { code: string } = MobData> {
    cards: T[] = []
...
export abstract class BaseController implements Mediator {
    ...
    protected loggingAvailable = true
    ...
    notify = (target: any, message: string, extra?: unknown): void => {
        if (this.resolve && this.enabled) {
            ...
            if (this.logAction && this.loggingAvailable) {
                const type =
                    extra && typeof extra === "string"
                        ? (extra as any)
                        : undefined
                this.logAction({
                    target: this.getObservableElCode(target),
                    action: message,
                    type,
                })
            }
    ...
    highlight = (el: SelectiveElement, config?: HighlightElementConfig) => {
        if (this.enabled) {
            const finalHighlightableStatus =
                this.highlightable &&
                (config
                    ? config.highlightable !== undefined
                        ? config.highlightable
                        : true
                    : true)
            const finalClickableStatus =
                this.clickable &&
                (config
                    ? config.clickable !== undefined
                        ? config.clickable
                        : true
                    : true)
            const finalAvailableStatus =
                this.avaialbe &&
                (config
                    ? config.available !== undefined
                        ? config.available
                        : true
                    : true)

            const canHighlight =
                finalClickableStatus ||
                finalHighlightableStatus ||
                finalAvailableStatus
    ...
    protected observe<D = unknown, T = unknown>(
        cb: (data: T) => void,
        formatter?: (com: Command<D>) => T,
        autoReturn = true
    ) {
        if (this.highlighted.size === 0 && autoReturn) {
            cb(
                formatter
                    ? formatter({ target: null as D, message: "null" })
                    : (null as T)
            )
        } else {
            this.resolve = cb
...
export class CardsController extends BaseController {
    ...
    makeDraw = () => {
        if (this.canDraw && this.resolve) {
            this.resolve({ message: LOG_ACTIONS.draw })
            this.reset()
            this.canDraw = false
        }
    }

    private highlightCardsSameAny = (
        data: any,
        cb: (data: any, card: MobData) => boolean,
        config?: HighlightElementConfig
    ) => {
    ...
    protected observe<D = unknown, T = unknown>(
        ...
        formatter?: (com: Command<D>) => T,
        autoReturn = true,
        from?: Extract<keyof CardsController, "hand" | "total" | "left">
    ): void {
        super.observe<D, T>(
            cb,
            (com) => {
                if (com.message !== LOG_ACTIONS.draw && from) {
                    this[from].remove(com.target as Card & MobData)
                }

                if (com.message === LOG_ACTIONS.select_card && this.logAction) {
                    this.logAction({
                        target: this.getObservableElCode(com.target as Card),
                        action: com.message,
                        type: "util",
                    })
                }
    ...
    getCardsSameGuildAsync = (
        guild: Guild,
        cb: CardWithoutResultHandler,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightCardsSameGuild(guild, { available: true, ...config })
        this.observe<Card | null, MobData | null>(
            cb,
            (c) => (c.target ? c.target.mobData : null),
            true,
            "hand"
        )
    }

    getCardsSameValueAsync = (
        value: number,
        cb: (data: MobData | null) => void,
        config?: HighlightElementConfigForGetActions
    ) => {
    ...
    getSomeCardCommandAsync = (
        cb: (command: Command) => void,
        config?: HighlightElementConfigForGetActions
    ) => {
        if (this.enabled) {
...
export class FieldsController extends BaseController {
    ...
    highlightNeighBoorEnemyFirstElemental = (
        field: Field,
        config?: HighlightElementConfig
    ) => {
        this.getEnemyNeighBoors(field).forEach((node) =>
            this.highlightElemental(
                this.getFirstFieldWithElementalInNode(node),
                config
            )
        )
    }

    getNeighBoorEnemyFirstElementalAsync = (
        field: Field,
        cb: FieldHandlerWithNoResult,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightNeighBoorEnemyFirstElemental(field, {
            available: true,
            ...config,
        })
    ...
    getAllNeighBoorEnemyFirstElementalsSync = (field: Field) => {
        return this.getEnemyNeighBoors(field)
            .map(this.getFirstFieldWithElementalInNode)
            .filter((f) => f) as Field[]
    }
    ...
    private highlightSomeElementalsInNode =
        (
            data: MobData,
            exceptions: Elemental[],
            isEqual: EqualFunction,
            config?: HighlightElementConfig
        ) =>
        (node: MapNode) => {
            node.fields.forEach((field) => {
                if (
                    field.elemental &&
                    isEqual(field.elemental, data) &&
                    !exceptions.includes(field.elemental)
                ) {
    ...
    highlightTeamElementalsByAny = (
        team: Team,
        data: MobData,
        isEqual: (el: Elemental, data: MobData) => boolean,
        exceptions: Elemental[],
        config?: HighlightElementConfig
    ) => {
        this.iterateThroughTeamNodes(
            this.highlightSomeElementalsInNode(
                data,
                exceptions,
                isEqual,
                config
    ...
    highLightTeamElementalsByGuild = (
        team: Team,
        data: MobData,
        exceptions: Elemental[],
        config?: HighlightElementConfig
    ) => {
        this.highlightTeamElementalsByAny(
            team,
            data,
            (el, data) => el.guild === data.guild,
            exceptions,
            config
        )
    }

    highlightTeamElementalsByValue = (
        team: Team,
        data: MobData,
        exceptions: Elemental[],
        config?: HighlightElementConfig
        ...
        this.highlightTeamElementalsByAny(
            team,
            data,
            (el, data) => el.maxHealth === data.value,
            exceptions,
            config
        )
    }

    highLightUserElementalsByGuild = (
        user: User,
        data: MobData,
        exceptions: Elemental[],
        config?: HighlightElementConfig
    ) => {
        this.highlightUserElementalsByAny(
            user,
            data,
            (el, data) => el.guild === data.guild,
            exceptions,
    ...
    highlightUserElementalsByValue = (
        user: User,
        data: MobData,
        exceptions: Elemental[],
        config?: HighlightElementConfig
    ...
    getTeamElementalsByValueAsync = (
        team: Team,
        data: MobData,
        exceptions: Elemental[],
        cb: FieldHandlerWithNoResult,
        config?: HighlightElementConfigForGetActions & { autoReturn?: boolean }
    ) => {
        this.highlightTeamElementalsByValue(team, data, exceptions, {
            available: true,
            ...config,
        })
    ...
    getUserElementalsByValueAsync = (
        ...
        this.highlightUserElementalsByValue(user, data, exceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb, !!config?.autoReturn)
    ...
    getTeamElementalByGuildAsync = (
        ...
        this.highLightTeamElementalsByGuild(team, data, exceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb, !!config?.autoReturn)
    ...
    getUserElementalByGuildAsync = (
        ...
        this.highLightUserElementalsByGuild(user, data, exceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb, !!config?.autoReturn)
    ...
    private highlightEmptyFieldHandler =
        (nodesExceptions: MapNode[], config?: HighlightElementConfig) =>
        (node: MapNode) => {
    ...
    highlightTeamAnyLastEmptyField = (
        team: Team,
        nodesExceptions: MapNode[],
        config?: HighlightElementConfig
    ) => {
        this.iterateThroughTeamNodes(
            this.highlightEmptyFieldHandler(nodesExceptions, config),
            team
        )
    }

    highlightUserAnyLastEmptyField = (
        user: User,
        nodesExceptions: MapNode[],
        config?: HighlightElementConfig
    ) => {
    ...
    getTeamAnyLastEmptyFieldAsync = (
        team: Team,
        nodesExceptions: MapNode[],
        cb: FieldHandler,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightTeamAnyLastEmptyField(team, nodesExceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb)
    ...
    getUserAnyLastEmptyFieldAsync = (
        user: User,
        nodesExceptions: MapNode[],
        cb: FieldHandler,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightUserAnyLastEmptyField(user, nodesExceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb)
    ...
    private getBeforeAfterElementalsSync = (
        field: Field,
        key: "next" | "prev"
    ) => {
    ...
    getNeighBoorLastEmptyFieldAsync = (
        field: Field | Field[],
        cb: FieldHandler,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightNeighBoorLastEmptyFields(field, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb)
    ...
    highlightNeighBoorLastEmptyFields = (
        field: Field | Field[],
        config?: HighlightElementConfig
    ) => {
        if (Array.isArray(field)) {
            ...
            this.getMyNeighBoors(field).forEach((node) =>
                this.highlightField(this.getLastFieldInNode(node), config)
            )
        }
    }

    highlightLastFieldInNode = (
        field: Field,
        config?: HighlightElementConfig
    ) => {
    ...
    getLastNeighBoorsAndCurrentEmptyFieldAsync = (
        fields: Field[],
        cb: FieldHandler,
        config?: HighlightElementConfigForGetActions
    ) => {
        if (fields.length === 2) {
            const isAllFieldsInOneNode = fields[0].parent === fields[1].parent

            if (isAllFieldsInOneNode) {
                this.highlightLastFieldInNode(fields[0], {
                    available: true,
                    ...config,
                })
            } else {
                this.highlightNeighBoorLastEmptyFields(fields, {
                    available: true,
                    ...config,
                })
            }
            ...
            this.highlightNeighBoorLastEmptyFields(fields[0], {
                available: true,
                ...config,
            })
        }
    ...
    highlightAnyElementalInThisNode = (
        field: Field,
        config?: HighlightElementConfig
    ) => {
        field.parent.fields.forEach(
            (f) => f !== field && this.highlightElemental(f, config)
        )
    }

    highlightEnemyAnyElementalInThisNode = (
        field: Field,
        config?: HighlightElementConfig
    ) => {
        field.parent.up!.fields.forEach((f) =>
            this.highlightElemental(f, config)
        )
    }

    getEnemyAnyElementalInThisNodeAsync = (
        field: Field,
        cb: FieldHandlerWithNoResult,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightEnemyAnyElementalInThisNode(field, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb)
    ...
    getAnyElementalInThisNodeAsync = (
        field: Field,
        cb: FieldHandlerWithNoResult,
        highlightable?: boolean
    ) => {
        this.highlightAnyElementalInThisNode(field, {
            clickable: true,
            highlightable,
        })
        this.formattedObserve(cb)
    ...
    private highlighAnyElementalHandler =
        (exceptions: Field[], config?: HighlightElementConfig) =>
        (node: MapNode) => {
            node.fields.forEach(
                (f) =>
    ...
    highlightTeamAnyElemental = (
        team: Team,
        exception: Field[],
        config?: HighlightElementConfig
    ) => {
        this.iterateThroughTeamNodes(
            this.highlighAnyElementalHandler(exception, config),
            team
        )
    }

    highlightUserAnyElemental = (
        user: User,
        exception: Field[],
        config?: HighlightElementConfig
    ) => {
    ...
    getTeamAnyElementalAsync = (
        team: Team,
        exceptions: Field[],
        cb: FieldHandlerWithNoResult,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightTeamAnyElemental(team, exceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb)
    ...
    getUserAnyElementalAsync = (
        user: User,
        exceptions: Field[],
        cb: FieldHandlerWithNoResult,
        config?: HighlightElementConfigForGetActions
    ) => {
        this.highlightUserAnyElemental(user, exceptions, {
            available: true,
            ...config,
        })
        this.formattedObserve(cb)
    ...
    iterateThroughAllNodes = (cb: (node: MapNode) => void) => {
        if (this.startNode.next)
            this.iterator.iterateThroughLine(
                this.startNode.next,
                cb,
                this.startNode,
                "next",
                    ...
                    if (
                        ...
                        if (
                            ...
                            selectedField = node.fields.find((field) => {
                                if (field.elemental) {
                                    return (
                                        field.elemental.mobData.code ===
                                        MobData.parseCode(
                                            guildCode as GuildCode,
                                            +value,
                                            +mobIndex
                                        )
                                    )
...
abstract class InGameStageUpdater<T extends InGameStage> {
    ...
    abstract updateAllState(
        state: Pick<ServerGameState, "teams" | "guilds" | "logs">
    ): void
}
...
export class GameRoomUpdater {
    ...
    updateProps = ({
        roomState: { props, password, type, name },
    }: ServerFinalState) => {
                ...
                if (userFeilds) {
                    ...
                    if (fields) {
                        ...
                        fields.forEach((field) => {
                            node.lastField!.setElemental(
                                ElementalMapper.toDomain(field)
                            )
                            node.addField()
                        })
                    }
...
export class DraftUpdater extends InGameStageUpdater<Draft> {
    private getGuilds = (guilds: GuildCode[]) =>
        GuildFactory.getInstance().createSeveral(guilds)
...
export class GameContentBuilder {
    ...
    parseUserDecks = (decs: Map<User, ServerUserDecks>) => {
        ...
        for (let entry of decs.entries()) {
            this.userCards.set(
                entry[0],
                CardsControllerMapper.toDomain(entry[1])
            )
        }
    ...
    addCardToCenter = () => {
        ...
        if (playersCount === 3) {
            let singlePlayer: User = this.users.teams.find(
                (t) => t.users.length === 1
            )!.users[0]
            ...
            this.gameField.iterateThroughAllNodes((node) => {
                if (
                    node.user &&
                    node.user[0] === singlePlayer &&
                    [1, 4].includes(node.index)
                ) {
                    const userCards =
                        this.userCards.get(singlePlayer)!.total.cards
            ...
            this.gameField.iterateThroughAllNodes((node) => {
                if (
                    node.user &&
                    node.user[0] !== firstPlayer &&
                    node.index === (playersCount === 4 ? 1 : 2)
                ) {
                    const userCards =
                        this.userCards.get(firstPlayer)?.total.cards
    ...
    createGameField = (usersFields?: Map<User, ElementalCode[][]>) => {
        ...
        const countFieldForCurrentCountPlayers =
            countFieldsPerPlayer[countOfPlayers] || 5
        const maxCountOfFieldsForGroup =
            maxGroupLength * countFieldForCurrentCountPlayers
        ...
        if (minGroupLength > 0) {
            groups.forEach((group, index, array) => {
                const countFieldsForUserOfGroup =
                    ...
                    for (let i = 0; i < countFieldsForUserOfGroup; i++) {
                        ...
                        if (usersFields) {
                            usersFields.get(user[0])![i]?.forEach((field) => {
                                lastNode.lastField!.setElemental(
                                    ElementalMapper.toDomain(field)
                                )
...
export class UserDraft {
    ...
    constructor(maxCount?: number) {
        this.maxChosenCount = Math.min(maxCount || this.maxChosenCount)
        makeAutoObservable(this)
    }

    setMaxCount = (value: number) => {
        this.maxChosenCount = Math.min(value, 4)
    }
...
export class UsersStorage {
    ...
    constructor(teams: Team[], currentUser?: User | null) {
        ...
        this.isViewer = !teams.some((team) =>
            team.users.some((u) => u.id === this.currentUser?.id)
        )
        makeObservable(this, {
    ...
    get groupedTeamsInRightOrder() {
        const foundTeam = this.currentUser
            ? this.teams.find((team) => team.hasUser(this.currentUser!))
            : undefined
        const currentTeam = foundTeam || this.teams[0]
        const currentTeamIndex = this.teams.indexOf(currentTeam)
        const currentUserIndex = foundTeam
            ? currentTeam.users.reduce(
                  (acc, user, i) =>
                      user.id === this.currentUser?.id ? i : acc,
                  -1
              )
            : 0
    ...
    private iterateAround<T>(
        array: Array<T>,
        startIndex: number,
        cb: (el: T, index: number, currentElIndex: number) => void
    ) {
        const start =
...
export class Turn extends UsersStorage {
    ...
    constructor(
        ...
        const maxCountOfUserPerTeam = this.teams.reduce(
            (acc, team) => Math.max(acc, team.playersCount),
            0
        )
        this.usersOrder = []
            ...
            teams.forEach((team) =>
                this.usersOrder.push([
                    team.users[i]
                        ...
                        : team.users[
                              i -
                                  Math.floor(i / team.users.length) *
                                      team.users.length
                          ],
                    team,
                ])
        ...
        const foundUserTuple = firstTurn
            ? this.usersOrder.find(([u]) => u === firstTurn)
            : undefined
        const userIndex = foundUserTuple
            ? this.usersOrder.indexOf(foundUserTuple)
            : undefined
        const newIndex =
            userIndex ?? Math.floor(Math.random() * (this.playersCount - 1))
    ...
    setRandomTurn = () => {
        this.currentTurnIndex = Math.floor(
            Math.random() * this.usersOrder.length
        )
    }
    ...
    setTurn = (turn: string) => {
        ...
        const user = foundUserOrder
            ? this.usersOrder.indexOf(foundUserOrder)
            : undefined
    ...
    next(sideEffect?: NextTurnSideEffect) {
        this.currentTurnIndex =
            this.currentTurnIndex === this.usersOrder.length - 1
                ? 0
                : this.currentTurnIndex + 1
        if (!this.isMyTurn) {
            this.logAction({
                action: LOG_ACTIONS.switch_turn,
                target: this.currentTurn.id,
                type: "extra",
            })
        }
        ...
        sideEffect
            ...
            : this.nextTurnSideEffect &&
              this.nextTurnSideEffect(this.currentTurn)
    }
...
export class PointsManager {
    ...
    setPointByTeamId = (id: string, value: number) => {
        const userTouple = this.turn.usersOrder.find(
            ([, team]) => team.id === id
        )
    ...
    hit = (killer: Field | null, victim: Field | null, damage: number) => {
        if (killer && victim && killer.elemental && victim.elemental) {
            ...
            if (victim.elemental.isDead) {
                ...
                const totalPoint =
                    killer.elemental.guild.pointsForKill > 0
                        ? victim.elemental.guild.extraPointsForDeath +
                          killer.elemental.guild.pointsForKill
                        : 0

                this.points.addPointToOppositeTeam(
                    victim.parent.user![1],
                    totalPoint
                )
                this.usersCards
                    .get(victim.parent.user![0])
                    ?.left.add(victim.elemental.mobData)
                victim.delete()
            }
        }
    }
...
export class Draw extends ActionStrategy {
    ...
    code = GameStrategyCodeType.draw
    ...
    start = () => {
        ...
        this.points.addPointToCurrentTurnPlayer(
            this.fieldsController.calculateControllerNodesByUser(
...
export abstract class SequentialStrategy extends ActionStrategy {
    ...
    constructor(
        ...
        this.data = data
        this.mounted = true
        this.canStartByGuild = this.checkStartByGuild()
        this.canStartByValue = this.checkStartByValue()
    }
    ...
    protected abstract checkStartByValue(): boolean
    protected abstract checkStartByGuild(): boolean
    ...
    protected get interactType() {
        return this.activeGuild ? InteractionType.guild : InteractionType.value
    }

    get isStarted() {
        return this.activeGuild || this.activeValue
    }

    resetHighlights = () => {
        const canReset = !this.activeGuild && !this.activeValue
...
export class Spawn extends SequentialStrategy {
    ...
    private readonly interactedFields: Field[] = []
    ...
    private choosePlace = (data: MobData | null, i: number, cb: Function) => {
        ...
        if (data) {
            ...
            if (i === 0) {
                this.fieldsController.getUserAnyLastEmptyFieldAsync(
                    this.turn.currentTurn,
                    [],
                    fieldHandler
                )
            } else if (i === 1) {
                this.fieldsController.getLastNeighBoorsAndCurrentEmptyFieldAsync(
                    this.interactedFields,
                    fieldHandler
                )
            } else {
                this.fieldsController.getLastNeighBoorsAndCurrentEmptyFieldAsync(
    ...
    private isEnd = (i: number) => {
        return (
            ...
            Math.min(
                MAX_SPAWN_ITERATION,
                this.cardsController.hand.cards.length
            )
        )
    }
    ...
    private iterate = (index: number, cb: Function) => {
        if (!this.isEnd(index)) {
            ...
            if (this.activeValue) {
                this.cardsController.getCardsSameValueAsync(
                    this.activeValue,
                    (data) => {
                ...
                this.cardsController.getCardsSameGuildAsync(
                    this.activeGuild,
                    (data) => this.choosePlace(data, index, recurse)
                )
            }
        } else {
            cb()
    ...
    startAction = () => {
        if (this.data) {
            this.cardsController.hand.removeByCode(this.data.code)
            this.logAction({
                action: LOG_ACTIONS.spawnBy(this.interactType),
                target: this.data.code,
            })
    ...
    protected checkStartByGuild() {
        this.cardsController.highlightCardsSameGuild(this.data.guild, {
            highlightable: false,
            clickable: false,
        })
        const highlightedCount = this.cardsController.highlighted.size
        this.cardsController.resetHighlights()
        return highlightedCount > 0
    }

    protected checkStartByValue() {
        this.cardsController.highlightCardsSameGuild(this.data.guild, {
            highlightable: false,
            clickable: false,
        })
...
export class Activation extends SequentialStrategy {
    ...
    private readonly intereactedElementals: Elemental[] = []

    private activateELemental = (field: Field | null, cb: Function) => {
        this.iterationStarted = true

        if (field && field.elemental) {
            const elemental = field.elemental!

            elemental.guild.activate(
                field!,
                this.fieldsController,
                this.killer,
                this.cardsController,
                () => {
                    this.intereactedElementals.push(elemental)
                    this.logAction({
                        type: "extra",
    ...
    private iterate = (index: number, cb: Function) => {
        if (index < MAX_ACTIVATION_ITERATION) {
            ...
            if (this.activeValue !== null) {
                this.fieldsController.getTeamElementalsByValueAsync(
                    this.turn.currentTurnTeam,
                    this.data,
                    this.intereactedElementals,
                    (field) => {
                ...
                this.fieldsController.getTeamElementalByGuildAsync(
                    this.turn.currentTurnTeam,
                    this.data,
                    this.intereactedElementals,
                    (field) => {
    ...
    startAction = () => {
        if (this.data) {
            ...
            this.logAction({
                action: LOG_ACTIONS.activateBy(this.interactType),
                target: this.data.code,
            })
    ...
    highlightByGuild = (): void => {
        this.fieldsController.highLightTeamElementalsByGuild(
            this.turn.currentTurnTeam,
            this.data,
            []
        )
    ...
    highlightByValue = (): void => {
        this.fieldsController.highlightTeamElementalsByValue(
            this.turn.currentTurnTeam,
            this.data,
            []
        )
    ...
    protected checkStartByGuild() {
        this.fieldsController.highLightTeamElementalsByGuild(
            ...
            this.data,
            [],
            { clickable: false, highlightable: false }
        )
        const count = this.fieldsController.highlighted.size
        this.fieldsController.resetHighlights()
        return count > 0
    }
...
export class DraftContentBuilder {
    ...
    formatGuildsPool = (withExtension: boolean) => {
        ...
        const initPool: GuildCode[] = shuffleArray([
            ...BASE_GUILDS_CODE,
            ...(needExtension ? ADD_GUILDS : []),
        ])
        const finalPool = withBan
            ? initPool
            : initPool.slice(0, playersCount * 2 + 6)
    ...
    addUsersRandomGuild = (count: number = 1) => {
        ...
        for (let i = 0; i < count; i++) {
            if (
                this.guilds.guilds.length >=
                Array.from(this.usersDraft.keys()).length
            ) {
                teams.forEach((team) => {
    ...
    setMaxCountOfGuildsPerUser = (maxGuildsCountPerPlayer?: number) => {
        const leftGuildsPerStage =
            this.maxCountPlayersPerTeam * this.users.teams.length
        const countOfBanStagesPerRound = this.draftStages.reduce(
            (acc, stage) => (stage === DraftStage.ban ? acc + 1 : acc),
            0
        )
        const countOfBanGuildsPerRound =
            countOfBanStagesPerRound * leftGuildsPerStage
        const countOfLeftGuildsPerRound =
            this.draftStages.length * leftGuildsPerStage
        const countOfRounds = Math.floor(
            this.guilds.guilds.length / countOfLeftGuildsPerRound
        )
        const totalCountOfBannedGuilds =
            countOfRounds * countOfBanGuildsPerRound
        const availableCountOfGuildForChoose =
            this.guilds.guilds.length - totalCountOfBannedGuilds
        const maxCountOfChosenGuildsPerTeam = Math.floor(
            availableCountOfGuildForChoose / this.users.teams.length
        )
        const maxCountOfChosenGuildsPerPlayer = Math.floor(
            maxCountOfChosenGuildsPerTeam / this.maxCountPlayersPerTeam
        )
        const countOfGuildsPerPlayer = maxGuildsCountPerPlayer
            ? Math.min(maxCountOfChosenGuildsPerPlayer, maxGuildsCountPerPlayer)
            : maxCountOfChosenGuildsPerPlayer
        ...
        this.users.teams.forEach((team) => {
            team.users.forEach((user) => {
                this.usersDraft
                    .get(user)
                    ?.setMaxCount(
                        (this.maxCountPlayersPerTeam / team.users.length) *
                            countOfGuildsPerPlayer
                    )
            })
        })
    ...
    formatDraftStages = (
        withBan: boolean,
        minGuildsCountPerPlayer?: number,
        template?: DraftStage[]
    ) => {
        if (
            template &&
            this.checkDraftTemplate(template, minGuildsCountPerPlayer)
        ) {
            this.draftStages = template
        } else {
            this.draftStages = withBan
                ? [DraftStage.pick, DraftStage.ban]
                : [DraftStage.pick]
        }
    ...
    get maxCountPlayersPerTeam() {
        return this.users.teams.reduce(
            (acc, t) => Math.max(t.users.length, acc),
            0
        )
    }
    ...
    checkDraftTemplate = (
        template: DraftStage[],
        minCountOfGuildsPerPlayer?: number
    ) => {
        ...
        if (template.length > 0) {
            while (
                countOfGuilds > 0 &&
                countOfGuilds >= leftGuildsPerTemplateStages
            ) {
        ...
        return (
            countOfGuilds >= 0 &&
            countChosenGuildsPerPlayer >= (minCountOfGuildsPerPlayer || 1)
        )
    }
...
export abstract class InGameStage<Prev = unknown, Def = unknown> extends Stage<
    Prev,
    Def
> {
    readonly turn: Turn
...
export class Draft extends InGameStage<DraftConfig, DefaultPicks> {
    ...
    nextStage = () => {
        this.currentStageIndex =
            this.currentStageIndex === this.stagesTemplate.length - 1
                ? 0
    ...
    setRandomDrafts = () => {
        while (!this.isEnd()) {
            if (this.guilds.guilds.length > 0) {
                this.currentUserDraft().choose(
                    this.guilds.takeRandomGuildSync()!
                )
                this.turn.next()
            }
        }
...
export class GameProcess extends InGameStage<
    Map<User, UserDraft>,
    DefaultProcessGameState
> {
    points: PointsManager
    ...
    constructor(
        ...
        isBigDaddy: boolean,
        logActionFunction?: LogAction,
        defaultState?: DefaultProcessGameState
    ) {
        super(
            InGameCode.process,
            controller,
    ...
    iterate = (cb: Function) => {
        if (!this.isEnd()) {
            if (this.turn.isMyTurn) {
                const canInteract = !this.turn.isGuest
                const needHighlight = true
                this.gameField.setHighlightable(needHighlight)
                this.gameField.setClickable(canInteract)

                this.currentDeck()?.setClickable(canInteract)
                this.currentDeck()?.setHighlightable(needHighlight)
            } else {
                this.gameField.setHighlightable(false)
                this.gameField.setClickable(false)
                this.currentDeck().setClickable(false)
                this.currentDeck().setHighlightable(false)
            }
    ...
    setDraw = () => {
        this.setStrategy(
            new Draw(this.currentDeck(), this.points, this.gameField, this.turn)
        )
    }
...
export class GameRoomPropsController {
    ...
    password: ValidationField = new ValidationField(
        initData.password,
        yup.string().min(3)
    )
    ...
    patchData = ({
        ...
        type,
        password,
    }: Partial<ServerRoomPropsPayload>) => {
    ...
    setPlayersCount = (value: 2 | 3 | 4) => {
        if ([3, 4].includes(value) && this.canEdit) {
            this.draftStages = [DraftStage.pick]
...
export class GameRoom extends Stage<any, GameRoomDefaultData, PrevState> {
    ...
    get canStart() {
        return (
            this.currentCountOfPlayers === this.props.playersCount &&
    ...
    get myTeam() {
        return (
            this.teams.find((team) =>
                team.users.some((user) => user.id === this.currentUser.id)
            ) || null
        )
    }
...
export type GameEndDefaultData = {
    winner: Team
    points: number
}

export class GameEnd extends Stage<User, GameEndDefaultData> {
    winner: Team
    points: number

    constructor(
        controller: IStageController,
        { winner, points }: GameEndDefaultData
    ) {
        super(StageCode.end, controller, true)
        this.winner = winner
        this.points = points
    }
...
export class Logger {
    ...
    protected onLogActionEffects: Record<string, LogSideEffect | null> = {}
    protected onAddLogEffects: Record<string, LogSideEffect | null> = {}
    ...
    addLogActionSideEffect = (key: string, action: LogSideEffect) => {
        this.onLogActionEffects[key] = action
    }

    removeLogActionSideEffect = (key: string) => {
        this.onLogActionEffects[key] = action
    }

    clearAllLogActionSideEffects = () => {
        this.onLogActionEffects = {}
    }

    addAddLogSideEffect = (key: string, action: LogSideEffect) => {
        this.onAddLogEffects[key] = action
    }

    removeAddLogSideEffect = (key: string) => {
        this.onAddLogEffects[key] = null
    }

    clearAllAddLogSideEffects = () => {
        this.onAddLogEffects = {}
    }

    logAction = (
        { type, action, instigator, target }: LogPayload,
        sideEffect?: (log: Log) => void
    ) => {
        if (this.enabled) {
            const defaultActionInstigator: LogInstigator = {
                target: null,
                type: LogInstigatorType.system,
            }
            ...
            const payload: Log = {
                ...
                timestamp: this.timer
                    ? { seconds: this.timer.seconds, UTC: this.timer.time }
                    : undefined,
                nested: [],
            }
            ...
            this.callSideEffects(this.onLogActionEffects, payload)
        }
    }

    private callSideEffects = (
        effects: Record<string, LogSideEffect | null>,
        log: Log
    ) => {
        getObjectKeys(effects)
            .filter((key) => effects[key])
            .forEach((key) => effects[key]!(log))
    }

    addLog = (log: Log, toLog?: string | null, ignoreSideEffect?: boolean) => {
        if (!ignoreSideEffect) {
            this.callSideEffects(this.onAddLogEffects, log)
        }
    ...
    move = () => {
        if (this.enabled && this.stageController) {
            if (this.stageController.getStage().code === InGameCode.draft) {
                this.moveInDraftStage(
                    new DraftFacade(this.stageController.getStage() as Draft)
                )
            }

            if (this.stageController.getStage().code === InGameCode.process) {
                this.moveInGameProcessStage(
                    new GameProcessFacade(
                        this.stageController.getStage() as GameProcess
                    )
                )
            }
        }
...
export class FoolishBot extends Bot {
    ...
    protected moveInGameProcessStage = (stage: GameProcessFacade) => {
        const spawn = () => {
            ...
            const availableElementalsForSpawn =
                stage.getCurrentTurnAvailableCards().length

            for (
                let i = 1;
                i < Math.min(availableElementalsForSpawn, MAX_SPAWN_ITERATION);
                i++
            ) {
        ...
        const activate = () => {
            const availableElementalsForActivation =
                stage.getAvailableFields().length

            for (
                let i = 0;
                i <
                Math.min(
                    availableElementalsForActivation,
                    MAX_ACTIVATION_ITERATION
                );
                i++
            ) {
                selectFieldsUnitActionIndexChanged()
        ...
        hand.forEach((card) => {
            data.set(
                card.mobData,
                this.getCountOfElementalsByGuildAndValue(stage, card.mobData)
            )
        })

        hand.forEach((card) => {
            const activateByGuild = () => {
                stage.activateCardByCode(
                    card.mobData.code,
        ...
        hand.forEach((card) => {
            const spawnByGuild = () => {
                stage.spawnCardByCode(card.mobData.code, InteractionType.guild)
                spawn()
            }
            const spawnByValue = () => {
                stage.spawnCardByCode(card.mobData.code, InteractionType.value)
                spawn()
            }
        ...
        const selectFieldsUnitActionIndexChanged = () => {
            ...
            while (
                stage.getSequentialActionIterationIndex() === currentIndex &&
                stage.getAvailableFields().length > 0
            ) {
                stage.selectRandomField()
            }
    ...
    private getCountOfElementalsByGuildAndValue = (
        ...
        stage.getCurrentTurnUserFields().forEach((fields) => {
            fields.forEach((field) => {
                if (field.elemental) {
                    if (
                        field.elemental.mobData.guild.code === data.guild.code
                    ) {
                        stat.guild += 1
                    }
...
abstract class GameAPI implements IGameAPI {
    ...
    abstract getGameState(): Promise<
        Pick<ServerGameState, "logs" | "teams" | "guilds">
    >
    ...
    abstract onChangeReadyStatus(
        cb: (userId: string, status: boolean) => void
    ): void
    abstract onEditProps(
        cb: (data: Partial<ServerRoomPropsPayload>) => void
    ): void
    ...
    abstract onSendMessage(cb: (message: ChatMessage) => void): void
    ...
    abstract onStartStage(
        cb: (stage: StageCode | InGameCode, data: ServerFinalState) => void
    ): void
...
export class WebSocketGameAPI extends GameAPI {
    ...
    pause = (
        ...
        return new Promise((res, rej) => {
            this.socket.emit(
                WEBSOCKET_EVENTS.togglePause,
                this.operateResponse(res, rej)
            )
        })
    }

    onGameStateChange(
        _: (data: Pick<ServerGameState, "logs" | "teams" | "guilds">) => void
    ): void {}
    ...
    private transformServerRedisStateToFinal = (
        data: RedisState
    ): ServerFinalState => {
        const isInGameStage = [InGameCode.process, InGameCode.draft].includes(
            data.stage as InGameCode
        )
    ...
    connect = (userId: string, cb: () => void): Promise<void> => {
        return new Promise((res) => {
            ...
            this.onWebsocketEvent(WEBSOCKET_EVENTS.sendMessage)

            this.socket.on(
                WEBSOCKET_EVENTS.startStage,
                (stage: StageCode | InGameCode, data: RedisState) => {
                    this.callAllCallbacks(WEBSOCKET_EVENTS.startStage, [
                        stage,
                        this.transformServerRedisStateToFinal(data),
                    ])
                }
            )
            res()
    ...
    private operateResponse =
        (res: Function, rej: (err: Error) => void) =>
    ...
    setReadyStatus(value: boolean): Promise<void> {
        return new Promise((res, rej) => {
            this.socket.emit(
                WEBSOCKET_EVENTS.changeReadyStatus,
                value,
                this.operateResponse(res, rej)
            )
        })
    ...
    onSendMessage(cb: (message: ChatMessage) => void): void {
        this.saveCallback(WEBSOCKET_EVENTS.sendMessage, cb)
    }
    ...
    editProps = async (
        payload: Partial<ServerRoomPropsPayload>
    ): Promise<void> => {
        this.socket.emit(
            WEBSOCKET_EVENTS.editRoomProps,
            payload,
            this.operateError
        )
    }
...
export class LocalGameApi extends GameAPI {
    ...
    private getLSState = (): ServerFinalState => {
        ...
        if (data) {
            return JSON.parse(data) as ServerFinalState
        }
    ...
    setGameState = async ({
        ...
        guilds,
        turnIndex,
    }: GameStatePayload): Promise<void> => {
        let prevState
        try {
            prevState = this.getLSState()
        } catch (e) {
            prevState = EMPTY_SERVER_FINAL_STATE
        }
...
export class GameState implements IStageController {
    ...
    constructor(sessionId: string, type: GameType, currentUser?: User | null) {
        ...
        this.api =
            type === GameType.local
                ? new LocalGameApi(sessionId)
                : new WebSocketGameAPI(sessionId)
        this.currentUser =
            currentUser || new User("guest", "Гость", "none", null)
        makeAutoObservable(this)
    }
    ...
    sendMessage = async (data: string) => {
        const message = new ChatMessage(data, this.currentUser!)
    ...
    private applyDefault = (data: ServerFinalState) => {
        ...
        const gameStage = new LogSyncrnoziedInGameStageControllerDecorator(
            new SyncronizedInGameStageControllerDecorator(
                new InGameStageController(this, isBigDaddy),
                this.api
            ),
            this.api
        )

        const ownerData = teams
            .map((t) => t.users)
            .flat()
            .find((u) => u.id === owner)!
        ...
        switch (stage) {
            case StageCode.room:
                ...
                gameRoom.setNext(gameStage as any)
                this.setStage(gameRoom)
                break
            case StageCode.game:
                gameStage.setDefaultData(gameDefaultData)
                this.setStage(gameStage as any)
                break
        }
        this.getStage().start()
    }
    ...
    start = async (data?: PrevState, onDeniAccess?: Function) => {
        if (data && this.type === GameType.local) {
            try {
                ...
                const gameStage = new SyncronizedInGameStageControllerDecorator(
                    new InGameStageController(this, true),
                    this.api
                )
                this.setStage(gameStage as any)
                this.getStage().start(data)
            }
        } else {
            try {
                if (this.currentUser) {
                    await this.api?.connect(this.currentUser.id, () => {})
                }

                this.api.onSendMessage((message) => {
                    this.messages.push(message)
                })
                ...
                this.api.onStartStage((stage, data) => {
                    if (stage !== StageCode.end) {
                        this.applyDefault(data as ServerFinalState)
                    } else {
                        const gameEndDefaultData: GameEndDefaultData = {
                            points: (data as ServerGameEndData).points,
                            winner: TeamMapper.toDomain(
                                (data as ServerGameEndData).team
                            ),
                        }
                        this.setStage(new GameEnd(this, gameEndDefaultData))
                    }
                })
            } catch (e: any) {
...
export class BaseInGameControllerDecorator implements IInGameStageController {
    protected controller: IInGameStageController
    constructor(controller: IInGameStageController) {
        this.controller = controller
    }

    get code() {
        return this.controller.code
    }

    setDefaultData = (data: any): void => {
        this.controller.setDefaultData(data)
    }
...
export class SyncronizedInGameStageControllerDecorator extends BaseInGameControllerDecorator {
    private updater: InGameStageUpdater<any> | undefined
    ...
    constructor(controller: IInGameStageController, api: ISyncGameAPI) {
        ...
        this.controller
            .getLogger()
            .addLogActionSideEffect("syncronizer", this.processLoggingAction)
    }
    ...
    private updateState = (
        ...
        if (stage) {
            const needSyncronizeUpdaterToStage =
                !this.updater || this.updater.stage.code !== stage.code

            if (needSyncronizeUpdaterToStage) {
    ...
    processLoggingAction = (log: Log) => {
        const logActionsForSync: string[] = [LOG_ACTIONS.switch_turn]

        if (log.action === LOG_ACTIONS.start_stage) {
            this.api.startStage(log.target as InGameCode, this.getEntityState())
        }
...
export class LogSyncrnoziedInGameStageControllerDecorator extends BaseInGameControllerDecorator {
    ...
    constructor(controller: IInGameStageController, api: ILoggingGameAPI) {
        ...
        this.controller
            .getLogger()
            .addLogActionSideEffect(
                "log_syncronizer",
                this.processLoggingAction
            )
    }
    ...
    processLog = (log: Log) => {
        ...
        if (canProcess) {
            ...
            if (stage) {
                const needSyncronizedLogreaderTostage =
                    !this.logReader ||
                    (this.logReader && this.logReader.code !== stage.code)

                if (needSyncronizedLogreaderTostage) {
    ...
    processLoggingAction = (log: Log) => {
        ...
        if (this.getTurnController().isMyTurn) {
            this.api.sendLog(log)
        }
    }
}

export class InGameStageController
    extends Stage<PrevState, InGameStageControllerDefaultData>
    implements IInGameStageController
{
    ...
    constructor(
        controller: IStageController,
        isBigDaddy: boolean,
        defaultData?: InGameStageControllerDefaultData
    ) {
    ...
    getEntityState = (): GameStatePayload => {
        ...
        const guilds =
            controller.getStage().code === InGameCode.draft
                ? GuildsPoolMapper.toEntity(
                      (controller.getStage() as Draft).guilds
                  )
                : []

        const feilds =
            controller.getStage().code === InGameCode.process
                ? FieldsControllerMapper.toEntity(
                      (controller.getStage() as GameProcess).gameField
                  )
                : new Map()
        ...
        return {
            turnIndex: controller.turn.currentTurnIndex,
            teams: controller.getTurnController().teams.map((team) => ({
                id: team.id,
                points:
                    controller.getStage().code === InGameCode.process
                        ? (
                              controller.getStage() as GameProcess
                ...
                users: team.users.map((user) => ({
                    ...
                    draft:
                        controller.getStage().code === InGameCode.draft
                            ? UserDraftMapper.toEntity(
                                  (
                                      controller.getStage() as Draft
                                  ).usersDraft.get(user)!
                              )
                            : emptyDraft,
                    cards:
                        controller.getStage().code === InGameCode.process
                            ? CardsControllerMapper.toEntity(
                                  (
                                      controller.getStage() as GameProcess
                                  ).usersCards.get(user)!
                              )
    ...
    processUserTurn = (user: User): void => {
        ...
        this.logAction({ action: LOG_ACTIONS.switch_turn })
    }
    ...
    start = (prev?: PrevState) => {
        ...
        if (this.defaultData) {
            ...
            const teams = this.defaultData.teams.map(
                ({ id, name, users, points }) => {
                    const team = new Team(
                        id,
                        users.map(({ fields, cards, draft, ...data }) => {
            ...
            this.turn.setCurrentTurnIndex(this.defaultData.turnIndex)
            this.turn.setNextTurnSideEffect(this.processUserTurn)

            this.configureAllBots()
            this.logger.setLogs(this.defaultData.logs)

            const gameProcess = this.createGameProcessInstance(
                gameProcessDefaultData
            )
...
class DraftLogReader implements LogReader {
    ...
    constructor(stage: Draft) {
        this.code = stage.code as InGameCode
    ...
    readLog({ action, target }: Log): void {
        switch (action) {
            ...
            default:
                ...
                const spawnOperation = LOG_ACTIONS.spawnBy(
                    InteractionType.guild
                ).split("_")[0]
                const activateOperation = LOG_ACTIONS.activateBy(
                    InteractionType.value
                ).split("_")[0]
...
export class Chat implements ChatController {
    ...
    messages: ChatMessage[] = []
    lastMessagesFailedOperations: Map<ChatMessage, Function> = new Map()
    currentUser: User

    constructor(user: User | null, api?: ChatAPI) {
        this.api = api || null
        this.currentUser = user || new User(uuid(), "Гость", "", null)
    }

    deleteMessageLocally = (message: ChatMessage) => {
        this.messages = this.messages.filter((m) => m !== message)
    }

    deleteMessage = (m: ChatMessage) => {
        this.messageProcess(m, this.api?.deleteChatMessage, () =>
            this.deleteMessageLocally(m)
        )
    }

    repeatMessageRequest(m: ChatMessage): void {
    ...
    private messageProcess = async (
        message: ChatMessage,
        asyncAction?: (m: ChatMessage) => Promise<void>,
        onSuccess?: Function,
        onError?: Function
    ) => {
        try {
    ...
    sendMessage = async (data: string, flag?: string) => {
        const message = new ChatMessage(data, this.currentUser, flag)
...
export class ChatMessage {
    ...
    controller: any | null = null
    loading: boolean = false
    mounted: boolean = false
    user: User
    flag?: string

    constructor(data: string, user: User, flag?: string, id?: string) {
        this.message = data
        this.user = user
        this.id = id || uuid()
        this.flag = flag
...
export class GameProcessFacade extends Facade {
    ...
    private interactWithCard = (
        card: Card | null | undefined,
        method: "activate" | "summon",
    ...
    activateCardByCode = (code: string, type: InteractionType) => {
        this.interactWithCard(
            this.getCurrentTurnAvailableCards().find(
                (card) => card.mobData.code === code
            ),
            "activate",
            type
        )
    ...
    spawnCard = (index: number, type: InteractionType) => {
        this.interactWithCard(
            this.getCurrentTurnAvailableCards()[index],
            "summon",
            type
        )
    }
    ...
    private interactWithSequentialAction = (
        cb: (action: SequentialStrategy) => void
    ) => {
        const action = this.process.action.strategy as SequentialStrategy
        if (action && action instanceof SequentialStrategy) {
            cb(action)
    ...
    selectFieldByCode = (code: string) => {
        this.selectElement(
            this.getAvailableFields().find((field) => field.code === code)
        )
    }
    ...
    getCurrentTurnAvailableCards = () => {
        console.log(this.process.currentDeck().highlighted)
    ...
    getCurrentTurnUserFields = () => {
        return this.process.gameField.getUserFieldsInArray(
            this.process.turn.currentTurn
        )
    }

    getCurrentTurnTeamFields = () => {
        return this.process.gameField.getTeamFieldsInArray(
            this.process.turn.currentTurnTeam
        )
    }
    ...
    getEnemiesForCurrentTurnUserFields = () => {
        return this.process.gameField.getEnemyFieldsForUserInArray(
            this.process.turn.currentTurn
        )
    }

    getEnemiesForCurrentTurnTeamFields = () => {
        return this.process.gameField.getEnemyFieldsForTeamInArray(
            this.process.turn.currentTurnTeam
        )
    }

    getCountPointsForDraw = () => {
        return this.process.gameField.calculateControllerNodesByUser(
            this.process.turn.currentTurn
        )
    }
}
...
abstract class PlayerAPI {
    ...
    abstract getGameSessionStartState(): Promise<
        Pick<PlayedGame, "startState" | "teams">
    >
}
...
class RemotePlayerAPI extends PlayerAPI {
    getGameSessionStartState = async () => {
        return privateApi
            .get<Pick<PlayedGame, "startState" | "teams">>("./")
            .then((data) => data.data)
    }
}

class LocalPlayerAPI extends PlayerAPI {
    getGameSessionStartState(): Promise<
        Pick<PlayedGame, "teams" | "startState">
    > {
        return new Promise((res, rej) => {
...
export class GamePlayer implements IStageController {
    ...
    constructor(session: string, type: GameType) {
        this.api =
            type === GameType.local
                ? new LocalPlayerAPI(session)
                : new RemotePlayerAPI(session)
    }
    ...
    start = async () => {
        try {
            ...
            this.stageController =
                new LogSyncrnoziedInGameStageControllerDecorator(
            ...
            this.startState.logs
                .filter(({ type }) => type === "util")
                .forEach((log) => {
    ...
    printKeyframe = (index: number) => {
        if (
            ...
            this.stageController &&
            index >= 0 &&
            index < this.startState.logs.length
        ) {
            this.applyState &&
                this.applyState(this.keyframes[index] as ServerGameState)
        }
    }
    ...
    stop(): void {
        if (this.startState) {
            ...
            this.timer.setSideEffect((time) => {
                const observableLog =
                    this.startState?.logs[this.currentLogIndex + 1]

                if (
                    observableLog &&
                    observableLog.timestamp &&
                    observableLog.timestamp.seconds <= time
                ) {
